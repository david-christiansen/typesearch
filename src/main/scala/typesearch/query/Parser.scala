...
class QueryParser(val lexical: QueryLexer = new QueryLexer) extends TokenParsers with PackratParsers with QueryTokens {
  ...
  lazy val funcType: PackratParser[QType] = "("~>(rep1sep(typ, ",")<~")")~(arrow~>typ) ^^ {
    case args~res => QFunc(args, res)
  } |
  typ~arrow~typ ^^ {
...
object TestQueryLexer extends QueryLexer with App {
  def test(): Unit = {
    print("----LEXER> ")
...
object TestQueryParser extends QueryParser with App {
  def test():Unit = {
    print("------PARSER> ")
    val input = Console.readLine()
    if (input != "q") {
